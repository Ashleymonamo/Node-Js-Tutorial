...
function uploadID() {
    ...
    function finalSubmission() {
       ...
       var passwordInput2 = document.getElementById("bttn2").value;
        ...
        //sending data to backend/endpoint:using POST method:use fetch API
            let application={ PersonalDetails:dataP,
        ...
        //post():creating data in a server
       const data={
           method:"POST",//METHOD WE'RE USING ON fetch()
           headers:{"Content-type":"application/json"},//tells web browser that we're working with json data
           body:JSON.stringify(application)//what's containing data,contains the actual data
        }
        //fetch has two parameters(1:the path/endpoint,2:data you're trying to send)
    fetch("/app8",data)//fetch:allows communication from frontend to backend
    .then((response)=>
    {
    return  response.json() //converts response into json
      ...
      location.href="/app9"+resp.message
